...
VALID_TLDS = set()
if os.path.exists("data/iana_tlds.json"):
    try:
        with open("data/iana_tlds.json", "r") as f:
            VALID_TLDS = set(json.load(f))
    except Exception as e:
        print(f"Warning: Failed to load iana_tlds.json: {e}")

if not VALID_TLDS:
    VALID_TLDS = {"com", "net", "org", "cn", "ai", "io", "cc", "top", "xyz", "tech", "vip", "fun", "site", "cloud", "pro", "work", "space", "ink", "shop", "me", "co", "link", "chat", "pub", "dev", "best", "app", "hk", "tw", "one", "in", "icu", "live", "today", "online", "run", "plus", "cx", "la", "so", "sh", "asia", "pw", "is", "fm", "vc", "su", "gg", "uk", "jp", "de", "us", "kr", "fr", "ca", "ru", "info", "biz", "mobi", "name", "art", "club", "store", "buzz", "moe", "ltd", "life", "xin", "win", "tv", "pics", "studio", "cool", "fan", "ren", "host", "love", "market", "sbs", "lol", "codes", "lat", "world", "cfd", "my", "mx", "mom", "ge", "gs", "vg"}
...
raw_data = {}
if os.path.exists("data/raw_fetched.json"):
    try:
        with open("data/raw_fetched.json", "r", encoding="utf-8") as f:
            raw_data = json.load(f)
    except Exception as e:
        print(f"Warning: Failed to load raw_fetched.json: {e}")
...
# 2. apisou pages
apisou_pages = {}
if os.path.exists("data/apisou_pages.json"):
    try:
        with open("data/apisou_pages.json", "r", encoding="utf-8") as f:
            apisou_pages = json.load(f)
    except Exception as e:
        print(f"Warning: Failed to load apisou_pages.json: {e}")
...
apiranking_pages = {}
if os.path.exists("data/apiranking_pages.json"):
    try:
        with open("data/apiranking_pages.json", "r", encoding="utf-8") as f:
            apiranking_pages = json.load(f)
    except Exception as e:
        print(f"Warning: Failed to load apiranking_pages.json: {e}")
...
redirects = {}
if os.path.exists("data/apiranking_redirects.json"):
    try:
        with open("data/apiranking_redirects.json", "r", encoding="utf-8") as f:
            redirects = json.load(f)
    except Exception as e:
        print(f"Warning: Failed to load apiranking_redirects.json: {e}")
...
# Write AIProxy.yaml (Clash Rule Provider - Classical)
clash_lines = [
    "# ==================================================================",
    "# 规则名称: 第三方 AI API 中转与代理分流规则 (AIProxy.yaml - Clash 纯海外/免翻分流版)",
    f"# 更新时间: {now_str}",
    f"# 包含域名: {len(sorted_domains)} 个独立中转服务商根域名",
    "# 过滤说明: 已剔除全部国内大厂、国内AI平台及全部 .cn 域名",
    "# 适用客户端: Clash / Clash Meta (Mihomo) / Clash Verge / Stash 等",
    "# 配置方式: rule-providers (behavior: classical)",
    "# 数据来源聚合:",
    "#   - GitHub: frank36512/aiapi",
    "#   - GitHub: howardpen9/awesome-ai-api-proxy",
    "#   - GitHub: Claws-ZH/awesome-ai-api",
...
with open("AIProxy.yaml", "w", encoding="utf-8") as f:
    f.write("\n".join(clash_lines) + "\n")
...
# Write AIProxy_Lite.yaml (Clash Rule Provider - Classical Lite)
clash_lite_lines = [
    "# ==================================================================",
    "# 规则名称: 精选高频第三方 AI API 中转分流规则 (AIProxy_Lite.yaml - Clash 纯海外/免翻分流版)",
    f"# 更新时间: {now_str}",
    f"# 包含域名: {len(lite_domains)} 个高频/多源收录中转站根域名",
    "# 过滤说明: 已剔除全部国内大厂、国内AI平台及全部 .cn 域名",
    "# 适用客户端: Clash / Clash Meta (Mihomo) / Clash Verge / Stash 等",
...
for d in lite_domains:
    clash_lite_lines.append(f"  - DOMAIN-SUFFIX,{d}")

with open("AIProxy_Lite.yaml", "w", encoding="utf-8") as f:
    f.write("\n".join(clash_lite_lines) + "\n")
